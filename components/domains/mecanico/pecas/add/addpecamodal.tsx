...
export default function AddPecaModal({
  isOpen,
  onClose,
  onSuccess,
}: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await authFetch('/api/mecanico/pecas/create', {
        method: 'POST',
      ...
      <div className="flex min-h-screen items-center justify-center px-4">
        <Dialog.Panel className="bg-white rounded-2xl shadow-xl p-10 w-full max-w-xl">
          <div className="mb-6 text-center">
            <Dialog.Title className="text-3xl font-bold">Adicionar Peça</Dialog.Title>
          </div>
