...
export default function Carrossel({ imagens }: CarrosselProps) {
  ...
  if (imagens.length === 0) {
    return (
      <div className="w-full h-80 bg-gray-200 flex items-center justify-center rounded-md mb-8">
        Sem imagens
      </div>
    )
  }
  ...
  const prev = () => {
    setIndex((old) => (old === 0 ? imagens.length - 1 : old - 1))
  }

  const next = () => {
    setIndex((old) => (old === imagens.length - 1 ? 0 : old + 1))
  }

  return (
    <div className="relative w-full max-w-4xl mx-auto mb-8 rounded-md overflow-hidden">
      <img
